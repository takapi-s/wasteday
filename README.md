...
- アプリ別（exe）・ブラウザドメイン別の使用状況を自動サンプリング（既定5秒）
...
- Q: どの情報が記録されますか？
  - A: 実行中アプリの実行ファイル名（例: `chrome.exe`）やブラウザドメイン、ウィンドウタイトル、操作状態（active/idle）のメタ情報と滞在秒数をローカルに保存します。
